...
export default function PresentationMeetingFormPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Get presentation meeting data (public endpoint)
        const formRes = await axios.get(`${API_URL}/public/forms/presentation-meeting/${leadId}`);
        ...
        if (err?.response?.status === 404) {
          setError('Form not found. Please ensure the lead has a customer requirement record.');
        } else {
          setError(`Unable to load form: ${errorDetail}`);
        }
      } finally {
        setLoading(false);
      }
  ...
  const handleSave = async (submitStatus: number = 1) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/public/forms/presentation-meeting/${leadId}`, dataToSave);
      setForm({ ...form, submit_status: submitStatus });

      if (submitStatus === 2) {
        setSuccessMessage('Demo schedule submitted successfully! You can still edit and update the form anytime.');
      } else {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (form && Object.keys(form).length > 0 && !saving) {
        ...
        axios.put(`${API_URL}/public/forms/presentation-meeting/${leadId}`, { ...form, submit_status: 1 }).catch(() => {});
      }
    }, 30000);
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: '1', minWidth: '300px', paddingRight: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <input
                  ...
                  onChange={(e) => updateForm('prefered_date', e.target.value)}
                  style={styles.input}
                />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <input
                  ...
                  onChange={(e) => updateForm('prefered_time', e.target.value)}
                  style={styles.input}
                  placeholder="HH:MM"
                />
              </div>
            ...
            <div style={{ flex: '1', minWidth: '300px', paddingLeft: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <input
                  type="text"
                  value={form.gi_remark || ''}
                  onChange={(e) => updateForm('gi_remark', e.target.value)}
                  style={styles.input}
                />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <select
                  value={form.timezone || ''}
                  onChange={(e) => updateForm('timezone', parseInt(e.target.value) || null)}
                  style={styles.select}
                >
